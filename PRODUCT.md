...
4. **Temporal navigation.** The day is the unit. Swipe left/right to move between days. Today is home. Beyond the day, there is one and only one other surface: a calendar reached by swiping down from the day grid, which exists solely to audit volume across days. It contains no information that isn't already in the daily entries, no analysis, no charts, no streak counters; tapping any cell returns you to that day's grid. There is no week view, no stats screen, no settings, no menu.
